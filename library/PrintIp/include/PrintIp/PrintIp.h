...
template<typename T>
std::enable_if_t<std::is_integral<T>::value, std::string> to_string(const T& value) {
...
*/
template<typename T>
std::enable_if_t<is_string_t<T>, std::string> to_string(const T& value) {
    return value;
}
...
*/
template<typename T>
std::enable_if_t<is_container_v<T>, std::string> to_string(const T& value){
    std::string result;
...
template<typename Type, size_t last>
struct build_tuple_string<Type, last, last> {
  static std::string get_tuple_value(const Type& value) {
    return std::to_string(std::get<last>(value));
  }
};
...
std::enable_if_t<is_all_same_v<Args...>, std::string> to_string(const std::tuple<Args...>& ip) {
  constexpr int num_args_tuple = std::tuple_size<std::tuple<Args...>>::value;
  return build_tuple_string<std::tuple<Args...>, 0, num_args_tuple - 1>::get_tuple_value(ip);
}
